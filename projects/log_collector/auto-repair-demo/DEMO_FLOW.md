...
併せて `log-summarizer` (LLM) が収集ログから概要を再生成し、D列(インシデント概要)を上書きする。

**演者トーク**: 「既存のlog_collectorスキルがログを引き、AIが概要をまとめました。ステータスは"ログ収集済み"で一旦停止しています。収集ログと概要を見て、続けて調査するか判断します」

★手動ゲート② — ここで状態機械は一旦停止する。演者が収集ログ(H列)・概要(D列)を確認してから、次の「▶ 続きを実行」ボタンで進める。

---

## 4. Step4-a: incident-analyzer が一次解析 (2〜3分)

演者が `ログ収集済み` の行で **▶ 続きを実行** ボタンを押すと、Claude Code サブエージェント `incident-analyzer` が起動。
...
─────  人が「▶調査＆改修案」ボタンを押す  ──────────────────────
13:01:35 ログ収集済み         log_collector+log-summarizer  D,H,M  ★手動ゲート②(収集ログ・概要を見て続行判断)
─────  人が「▶続きを実行」ボタンを押す  ──────────────────────
