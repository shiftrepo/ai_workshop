...
1. **ブランチ切り** — ブランチ名は **TrackID を含めてユニーク化** する。分岐元・PRベースは **`main`** (demo資材は既にmainにマージ済み)。以下を順に実行:
   ```bash
   cd "$(git rev-parse --show-toplevel)"   # git のトップ (作業環境によりパスが変わるため固定しない)
   BRANCH="fix/inc001-<trackId小文字>"     # 例: fix/inc001-714a606
   git fetch origin main
   git branch -D "$BRANCH" 2>/dev/null || true   # 前回デモの残骸を削除
   git checkout -b "$BRANCH" origin/main
   ```
   - **デモ運用上、リポジトリは常に `main` ブランチにいてクリーンな状態が前提**。`git checkout -b` が「未コミット変更で切り替えられない」と出た場合は、下記「代替フロー」に切り替える。
...
4. `git stash pop`
5. `git add <対象ファイル>` → commit → push → PR (base=main)
...
- リポジトリルート: `git rev-parse --show-toplevel` で取得する (作業環境によりパスが変わるため固定しない)
- **fix ブランチは `origin/main` から切る** (demo資材は main にマージ済み)
- **PR は `main` 向けに作成する** (`--base main`)
