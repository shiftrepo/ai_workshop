...
`要承認` で状態機械は停止する。ブラウザで `http://ec2-54-88-196-71.compute-1.amazonaws.com:4001` (内部からは `http://localhost:4001`) を開き、改修案 (I 列) を確認したら:

1. **承認者** カラム (J列) に自分の名前を入れる (Enter または blur で即保存)
2. **ステータス** カラム (E列) のドロップダウンで **`PR作成待ち`** を選ぶ (即保存)

Web UI で編集した値は即座に Excel ファイルに書き込まれる。3〜5 秒で `excel-driver.js` が検知し、`pr-publisher` エージェントを起動:
- `fix/inc<番号>-...` ブランチを切る
- 改修案の diff を実コードに適用
- テスト実行
...
| ステータス | 起動対象 | 次のステータス |
|-----------|----------|---------------|
| 情報収集中 | 既存 `log-collector-skill` | 解析待ち |
| 解析待ち | `claude --agent incident-analyzer` | 改修案作成待ち |
| 改修案作成待ち | `claude --agent repair-planner` | 要承認 |
| 要承認 | — (人手ゲート) | — |
| PR作成待ち | `claude --agent pr-publisher` | 対応完了 |
