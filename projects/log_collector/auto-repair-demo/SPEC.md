...
```
   ...
   │  │INC1│...      │Track:A  │AI   │情報収集中     │          ││ ←起点
   │  └────┴─────────┴─────────┴─────┴──────────────┴──────────┘│
   └──────────────────────────────────────────────────────────────┘
                     │ orchestrator が状態遷移を監視
                     v
     ┌──────────────────┬──────────────────┬──────────────────┐
     │ ステータス       │ 起動エージェント │ 書き戻し先        │
     ├──────────────────┼──────────────────┼──────────────────┤
     │ 情報収集中       │ log_collector    │ 調査状況(ログ添付)│
     │ 解析待ち         │ incident-analyzer│ 一次解析結果      │
     │ 改修案作成待ち   │ repair-planner   │ 改修案+テスト計画 │
...
```
    [新規]                     ← 監視ツールが起票 (Step3 手前)
      │
      v ステータス="情報収集中"
    [log_collector]            ← 既存Skill が SSH で該当ログを引く
      │
      v ステータス="解析待ち"    ← log_collector が自動更新
    [incident-analyzer]        ← 一次解析エージェント (New)
      │
      v ステータス="改修案作成待ち"
    [repair-planner]           ← 改修案エージェント (New)
      │
      v ステータス="要承認"     ← 人が確認するために停止
      │
      │ (人が Excel を編集)
      │  ステータス="PR作成待ち" ← 起点。人がここに進めた時だけ次段発火
      v
...
2. **人の承認ゲートは1つ** — `要承認 → PR作成待ち` の1箇所のみ (Issue #22 の「改修案の確認後改修して、PRを発行する」に対応)。
3. **エージェントは冪等** — 同じステータスの行が2回流れても副作用が二重にならないよう、L列(最終更新)で重複判定。
4. **Issue #22 の指示準拠** — 「エクセルのインシデントファイルのインシデント状態を起点に実行」を pr-publisher の発火条件で忠実に実装。
...
- **入力**: Excel の "解析待ち" 行 + G列の収集ログサマリで示された生ログ.xlsx
- **やること**:
  1. ログを読み、症状 (Symptom) と最も可能性の高い原因 (RootCauseHypothesis) を1〜3件挙げる
  2. 影響範囲 (どのエンドポイント/機能が壊れているか) を特定
  3. 再現手順 (curl等) を書き出す
- **出力**: Excel H列 に構造化テキスト + ステータスを "改修案作成待ち" に更新
- **モデル**: Sonnet (通常) / Opus (難しい障害のみ)

#### B. `repair-planner` — 改修案提示エージェント
- **入力**: Excel の "改修案作成待ち" 行 + H列の解析結果 + 対象コード (demo-app/)
- **やること**:
  1. 修正すべきファイル/行を特定
...
const STATE_HANDLERS = {
  '情報収集中':     { agent: 'log_collector',      nextStatus: '解析待ち' },
  '解析待ち':       { agent: 'incident-analyzer',  nextStatus: '改修案作成待ち' },
  '改修案作成待ち': { agent: 'repair-planner',     nextStatus: '要承認' },
  // '要承認' は人手待ちのため handler なし
  'PR作成待ち':     { agent: 'pr-publisher',       nextStatus: '対応完了' },
};
