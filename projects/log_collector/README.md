...
**Web UI のボタン** (各段は手動ゲート、自動では進まない):

| 行のステータス | ボタン | 動作 |
|---------------|--------|------|
| 情報収集中 | **▶ ログ収集** | log_collector が該当ログ収集 → `インシデント検出` で停止 (AI解析なし) |
| インシデント検出 | **▶ 調査＆改修案** | incident-analyzer + repair-planner を実行 → `要承認` で停止 (数分) |
...
| 遷移 | 書き込み担当 | 書き込み列 |
|------|-------------|-----------|
| 情報収集中 → インシデント検出 | log-collector-skill (既存) | G 収集ログサマリ |
| インシデント検出 → 解析済み | `incident-analyzer` サブエージェント (Sonnet 5) | H 一次解析結果 (~800-1300文字) |
