...
```
   ...
   └──────────────────────────────────────────────────────────────┘
     ...
     ├──────────────────┼──────────────────┼──────────────────┤
     │ インシデント検出 │ log_collector    │ 収集ログ+概要     │ ★起票=手動ゲート①
     │ ログ収集済み     │ incident-analyzer│ 一次解析結果      │ ★手動ゲート② (収集ログ・概要を見て続行判断)
...
| 列 | ヘッダー | 記入者 | 内容 |
|----|----------|--------|------|
| H | 収集ログ | log_collector | 各サーバから取得したTrackID一致ログ行をそのまま結合して格納 (`[サーバ名 ログパス] ログ本文` 形式、加工・集計はしない) |
...
```
    [新規]                       ← 監視ツール(watchdog)がバグ発火を検知
      ...
      │ (人が「▶ 調査＆改修案」ボタンを押す)
      v
    [log_collector]              ← 既存Skill が SSH で該当ログ(TrackID一致)を引く (H列)
    [log-summarizer]             ← 収集した生ログから概要を生成しD列を上書き (LLM)
      │
      v ステータス="ログ収集済み"  ★手動ゲート② — 収集ログ・概要を見て続行を判断
      │
      │ (人が「▶ 続きを実行」ボタンを押す)
      v
...
3. **手動ゲートは3つ** —
   - `インシデント検出` (ゲート①): 起票直後。人が「▶調査＆改修案」を押すまで重い処理 (ログ収集) は走らない。
     同一事象が TrackID 単位で複数起票されるため、人が取捨選択してから進める。
   - `ログ収集済み` (ゲート②): 検出→ログ収集→概要記載 (D列・H列) までを一連の動作として実行した直後に停止する。
     収集ログの生本文と、そこからLLMが生成した概要を見て、一次解析・改修案作成に進めるかを人が判断する。
   - `要承認` (ゲート③): 改修案の確認後にのみ PR を発行 (Issue #22 の「改修案の確認後改修して、PRを発行する」に対応)。
4. **エージェントは冪等** — 同じ行が2回流れても M列(最終更新)で重複判定。
5. **`▶ 調査＆改修案` ボタン** は「インシデント検出 → ログ収集済み」を実行し、次の手動ゲート `ログ収集済み` で停止する。
   続けて一次解析・改修案作成に進めるには「▶ 続きを実行」ボタンで `要承認` まで進める。
...
- **発火**: `ログ収集済み` 行で人が「▶ 続きを実行」ボタンを押したとき (手動ゲート②)。収集ログ・概要を確認した上での判断
