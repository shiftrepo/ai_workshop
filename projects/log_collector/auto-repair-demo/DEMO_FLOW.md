...
```
| ID  | Time | 概要 | 担当 | ステータス | 調査状況 | ログ | 解析 | 改修案 | 承認者 | PR | 更新 |
|-----|------|------|------|-----------|----------|------|------|--------|--------|----|----- |
| (空)                                                                                     |
```
...
|--------|-----------|----------------------|-------------------------------------------|----------------|
| INC001 | 解析待ち   | ログ12件収集         | log-collection-result_..._13-01-30.xlsx  | 2026-07-17 13:01:35 |
```

**演者トーク**: 「既存のlog_collectorスキルがログを引いてExcelに添付、ステータスを"解析待ち"に進めました」

---

## 4. Step4-a: incident-analyzer が一次解析 (2〜3分)

excel-driver が "解析待ち" を検知 → Claude Code サブエージェント `incident-analyzer` を起動。
...
**PR本文** (`.pr-body-inc001.md`) には Excelの H,I,J 列がそのまま貼られる:
```markdown
## 概要
...
**Excel K列に PR URL を書き戻し + ステータス `対応完了` に更新**:
```
| ID     | ステータス | PR URL                                             | 更新 |
|--------|-----------|----------------------------------------------------|------|
...
13:01:35 解析待ち             log_collector        F,G,L
13:02:50 改修案作成待ち       incident-analyzer    H,E,L
