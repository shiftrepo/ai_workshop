...
stateDiagram-v2
    ...
    インシデント検出 --> ログ収集済み: ▶調査＆改修案①<br/>log-collector-skill でSSHログ収集
    ログ収集済み --> 解析済み: incident-analyzer 一次解析
...
- **▶ 調査＆改修案 ボタン①**: `インシデント検出` → `ログ収集済み` → `解析済み` → `要承認` を一気に実行し、要承認で停止 (数分)。
