...
SSH_KEY_DIR="../sample-data"
SSH_KEY_PATH="$SSH_KEY_DIR/log_collector_key"

mkdir -p "$SSH_KEY_DIR"

if [ ! -f "$SSH_KEY_PATH" ]; then
    echo "  📝 Generating SSH key pair..."
    ssh-keygen -t rsa -b 2048 -f "$SSH_KEY_PATH" -N "" -C "log-collection-fixed-key"
...
PATTERN_FILE="../../client/examples/log-patterns.json"
if [ ! -f "$PATTERN_FILE" ]; then
    echo "  ⚠️  Log patterns file missing: $PATTERN_FILE"
else
    echo "  ✅ Log patterns file exists: $PATTERN_FILE"
fi
...
docker build -t log-collector-tool:issue15-latest -f Dockerfile ../.. --no-cache
