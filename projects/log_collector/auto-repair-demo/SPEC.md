...
```
   ...
   └──────────────────────────────────────────────────────────────┘
     ...
     │ インシデント検出 │ (手動ゲート①)    │ ―                 │
     │ (↑で調査ボタン)  │ incident-analyzer│ 一次解析結果      │
...
```
    [新規]                       ← 監視ツール(watchdog)が起票
      │
      v ステータス="情報収集中"
    [log_collector]              ← 既存Skill が SSH で該当ログを引く
      │
      v ステータス="インシデント検出"  ★手動ゲート① — ここで停止
      │
      │ (人が「▶ 調査＆改修案」ボタンを押す)
      v
...
2. **手動ゲートは2つ** —
   - `インシデント検出` (ゲート①): ログ収集の後、重い AI 処理 (解析/改修) の前で停止。
     同一事象が TrackID 単位で複数起票されるため、人が取捨選択してから解析に進める。
